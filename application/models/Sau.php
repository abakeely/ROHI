...
class Sau extends MY_Controller {
	...
	public function updateAllVisite(){
		$oVisite["visite_heureSortie"]	= date("H:i:s");
		$this->Visiteur->updateAllVisite($oVisite);
	}
	...
	public function listeVisiteToDay (){
		...
		$oSmarty->assign("oListe", $oListe);
		$oSmarty->assign("oListeSortie", $oListeSortie);
		$oSmarty->assign("iCountListe", sizeof($oListe));
		$oSmarty->assign("iCountSortie", sizeof($oListeSortie));
		$oSmarty->assign('zBasePath', base_url());
		...
		$toReturn = array();
		$toReturn['zFormListeVisiteur'] = $zFormListeVisiteur;
		$toReturn['iCount'] = sizeof($oListe) + sizeof($oListeSortie);
	...
	public function login ($_zHashModule = FALSE, $_zHashUrl = FALSE) {
		...
		if($iRet == 1){
			...
			$toPoste = $this->Visiteur->get_all_poste();
	...
	public function visiteurs ($_zHashModule = FALSE, $_zHashUrl = FALSE, $_iCurrPage=1) {
		...
		$oData['menu'] = MENU_ESPACE_ECHANGE;
		$oData['iNotificationAffiche'] = 1;
		$oData['oUser'] = $oUser;
		$oData['oCandidat'] = $oCandidat;
		...
		if($iRet == 1){	
			...
			$oListe = $this->Visiteur->get_all_visiteur('','',3);
			$oData['toPorte']	= $toPorte;
			$oData['toBadge']	= $toBadge;
			$oData['oListe']	= $oListe;
			$oData['iCount']	= sizeof($oListe);
	...
	public function transactions ($_zHashModule = FALSE, $_zHashUrl = FALSE, $_iCurrPage=1) {
		...
		if($iRet == 1){	
			...
			if ($zDateDebutInit != '' && $zDateFinInit != '' && $iBadgeId != ''){
				$toGetAllTransaction = $this->Transaction->get_transactionVisiteur($iBadgeId,$zDateDebutInit,$zDateFinInit,$this,$iNbrTotal, NB_PER_PAGE, $_iCurrPage);
