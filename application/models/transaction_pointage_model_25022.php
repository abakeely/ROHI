...
class Transaction_pointage_model extends CI_Model {
	...
	public function updateBadge($oData, $_iBadgeId){
		...
		$this->db->update('badge', $oData, "badge_id = $_iBadgeId");
		return $_iGcapId ; 
	}
					...
					foreach($oDateRange as $zDdate){
						...
						foreach ($tzDiff as $zDiff) {
							...
							if (isset($toInformation[2]) && ($toInformation[2] != '')) {
								...
								$zDateEntrer = str_replace(".0000000","",$toInformation[0]);

								if ($zDdate->format("Y-m-d") == date("Y-m-d",strtotime($zDateEntrer))){ 

									$toAssign->tzDiff[$i]["zDateEntree"]	= $zDateEntrer;
									$toAssign->tzDiff[$i]["zDateSortie"]	= str_replace(".0000000","",$toInformation[1]);
	...
	function insertGcapZkGcap(){
		...
		$zSql= "SELECT g.*,IF(STRCMP(matricule,'ECD'),matricule,SUBSTRING(REPLACE(cin,' ',''), 4, 9)) AS matricule FROM $zDatabaseGcap.gcap g LEFT JOIN $zDatabaseOrigin.candidat c ON g.gcap_userSendId = c.user_id " ;
...
/*********************** A commenter en ligne ***********************************************/
	...
	public function CheckImCarte($_iBadgeNumber,$_iNumCarte){
		...
		$iRet = 0 ;
		$zQuerySqlServer = "SELECT * FROM [ZKAccess2018].[dbo].[USERINFO] WHERE [Badgenumber] = '".(int)$_iBadgeNumber."' AND [CardNo]='".$_iNumCarte."'";
	...
	public function getImCarte($_iBadgeNumber){
		...
		$iRet = 0 ;
		$zQuerySqlServer = "SELECT CardNo FROM [ZKAccess2018].[dbo].[USERINFO] WHERE [Badgenumber] = '".(int)$_iBadgeNumber."'";
		...
		if ($_iSeuil == 1 AND $_iNbrRetard > 0 && $_zHeureSeuil != '') {
			...
			$zQuerySqlServer = "SELECT t.pin,t.pin,
				CAST((
					  ...
					  FROM [ZKAccess2018].[dbo].[acc_monitor_log] t3
	...
	public function getAllUserSubordonnees ($_oUser,$_oCandidat,$_iUserId, $_iCompteActif, &$zNotIn="",$_isPointage=TRUE) {
				...
				if ($_oUser['serv'] > 0) {
					...
					if ($_oCandidat[0]->division > 0 && $_oCandidat[0]->division < '500') {
						...
						$zSql .= "INNER JOIN $zDatabaseOrigin.module m ON m.id = c.division
							...
							" ;
					} elseif ($_oCandidat[0]->service > 0 && $_oCandidat[0]->service < '700') {
						/* même service */
		...
		case COMPTE_AUTORITE :
				...
				if ($_oCandidat[0]->service !='') {
					/* même service */
					$zSql .= "INNER JOIN $zDatabaseOrigin.service s ON s.id = c.service
						WHERE s.id  = (SELECT service FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat[0]->id.") AND c.region_id = ".$_oCandidat[0]->region_id." 
						" ;
				} else {
					/* même direction */
					$zSql .= "INNER JOIN $zDatabaseOrigin.direction d ON c.direction = d.id
			...
			$zQuerySqlServer = "  SELECT CONCAT('',(
												...
												CASE 
													WHEN (gcap_demiJournee = 1 AND gcap_MatinSoir = 1 AND nbr = 1)
														THEN ' 12:00:00.0000000'
													ELSE ' 08:00:00.0000000' 
												END AS varchar
					...
					if ($zValue != 0) {
						...
						foreach ($toCandidatUser as $oCandidatUser)
						{
							 ...
							 if ((int)$oCandidatUser["matricule"] == (int)$iUserId){
								...
								if (isset($toListe[$iKeyB]) && sizeof($toListe[$iKeyB])>0){
									$iKeyB += 1 ; 
								}
...
/*********************** A commenter en ligne ***********************************************/
	...
	function __2__TempsDeTravailDesAgents($_toCandidatUser, $_oCandidat, $iResultSearch, $_zDateDebut, $_zDateFin, $_this, $iCompteTotal,$iReturn=0,&$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1) {
			...
			$zQuerySqlServer = "SELECT SENSORID,time,USERID,CHECKTYPE FROM [ZKAccess2018].[dbo].[USERINFO] userInfo
			INNER JOIN [ZKAccess2018].[dbo].[acc_monitor_log]  ON ckIn.USERID = userInfo.USERID
	...
	function TempsDeTravailDunAgentAvecDenominateur($_zInMatriculeUser, $_zDateDebut, $_zDateFin,&$iDenominateur, $_this, $_iAnnee="2018") {
			...
			/*$zQuerySqlServer = "SELECT SENSORID,time,userInfo.USERID,CHECKTYPE FROM [ZKAccess2018].[dbo].[USERINFO] userInfo
			INNER JOIN [ZKAccess2018].[dbo].[acc_monitor_log]  ON ckIn.USERID = userInfo.USERID
			WHERE (userInfo.pin IN (". $zInMatriculeUser .")) AND ".$zInsert."
	...
	function ___TempsDeTravailDesAgents($_toCandidatUser, $_oCandidat, $iResultSearch, $_zDateDebut, $_zDateFin, $_this, $iCompteTotal,$iReturn=0,&$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1) {
			...
			( SELECT ABS(datediff(second, (SELECT TOP 1 time  FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t3 WHERE (t3.USERID = t.USERID AND ".$zInsert." AND CHECKTYPE='O') GROUP BY time ORDER BY time DESC), (SELECT TOP 1 time  FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t4 WHERE (t4.USERID = t.USERID AND ".$zInsert." AND CHECKTYPE='I') GROUP BY time ORDER BY time ASC)))) as diff
			FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t
			WHERE (USERID IN (". $zInMatriculeUser .")) AND ".$zInsert."
			...
			if ($iReturn == 1) {
				...
				$zQuerySqlServer2 = "SELECT SENSORID,time,t.USERID,CHECKTYPE,( SELECT ABS(datediff(second, (SELECT TOP 1 time  FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t3 WHERE (t3.USERID = t.USERID AND ".$zInsert." AND CHECKTYPE='O') GROUP BY time ORDER BY time DESC), (SELECT TOP 1 time  FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t4 WHERE (t4.USERID = t.USERID AND ".$zInsert." AND CHECKTYPE='I') GROUP BY time ORDER BY time ASC)))) as diff FROM [ZKAccess2018].[dbo].[USERINFO] userInfo as t
				WHERE (USERID IN (". $zInMatriculeUser .")) AND ".$zInsert."
...
/*********************************************** A decommenter en ligne ***********************************************************/
		...
		/*$zQuerySqlServer = " SELECT time,pin,event_point_name as terminal, CAST(
							 CASE 
								  ...
								  ELSE 'I' 
							 END AS varchar) as CHECKTYPE
							 FROM [ZKAccess2018].[dbo].[acc_monitor_log]
							 WHERE (pin IN (".$zInMatriculeUser.")) AND ".$zInsert."
							GROUP BY time,event_point_name,pin ORDER BY time";*/
	...
	public function updateMahandry($iNbrHeure = 10, $zWhere = ''){
		...
		$zQuerySqlServer = "SELECT LID,DevID,time,USERID,CHECKTYPE FROM [ZKAccess2018].[dbo].[USERINFO] userInfo " ; 
		...
		foreach ($toGetAllTransaction as $oGetAllTransaction)
		{
			 ...
			 $sql2 = "UPDATE [ZKAccess2018].[dbo].[USERINFO] userInfo SET time = '".$oReturn->timeUpdate.".000' WHERE LID = '". (int)$oGetAllTransaction['LID'] ."' AND DevID = '".(int)$oGetAllTransaction['DevID']."' AND 
	...
	public function SavePointageDirect($_iMatricule,$_zDateEntree,$_zDateSortie) {
		...
		$zSqlEntree = " insert into [ZKAccess2018].[dbo].[USERINFO] userInfo ([USERID],[time],[CHECKTYPE]) values ('".$_iMatricule."','".$_zDateEntree.".000',1)";

		$this->executeQuery($zSqlEntree);

		$zSqlSortie = " insert into [ZKAccess2018].[dbo].[USERINFO] userInfo ([USERID],[time],[CHECKTYPE]) values ('".$_iMatricule."','".$_zDateSortie.".000',0)";
	...
	public function setPdfRapports ($_oData,$_toListe, $_zDateDebut, $_zDateFin) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			$oPdf->SetX(10);
			$oPdf->Cell(100,7,utf8_decode('MINISTÈRE DES FINANCES ET DU BUDGET'),0,0,'C',1);
	...
	public function UpdateAcces($iUserId, $oUserChange){
		...
		if($this->db->insert('moderation', $oUserModeration)){
			$iModeration =  $this->db->insert_id();
	...
	public function getAllModeration($_zUserId,$iArchive, $iStatut, &$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1, $_zSortSens = "DESC", $_zFieldOrder = "moderation_date") {
        ...
        $toDataCount = mysql_query ($zQueryDataCount) ;

        while ($toRow = mysql_fetch_array ($toDataCount))
        {
            $_iNbrTotal = $toRow [0] ;
        }
	...
	public function suppressionDoublon(){
		...
		if ($zInDoublon != '') {
			$this->db->query('DELETE FROM user  WHERE id IN ('.$zInDoublon.')');
			$this->db->query('DELETE FROM candidat  WHERE user_id IN ('.$zInDoublon.')');
		}
