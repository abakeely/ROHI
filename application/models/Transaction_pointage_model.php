...
class Transaction_pointage_model extends CI_Model {
	...
	public function __construct(){
		...
		global $db;
		$this->load->database('transactions', TRUE);
	}

	public function executeQuery($_zSql){
		error_reporting(E_ALL);
		$oConnection	= $this->load->database('newpointage', TRUE);
		/*$zError			= odbc_exec($oConnection->conn_id,$_zSql) ; 
		$oResult		= odbc_do ($oConnection->conn_id,$_zSql);*/
		$zQuery = $oConnection->query($_zSql);
	...
	public function getCongePeriode ($_iUserId, $_zDateDebut, $_zDateFin,$_this){
		...
		if ($_zDateDebut != $_zDateFin) {
			$zSql .= " AND ('$_zDateDebut' BETWEEN gcap_dateDebut AND gcap_dateFin AND matricule = '$_iUserId') " ; 
			$zSql .= " OR ('$_zDateFin' BETWEEN gcap_dateDebut AND gcap_dateFin AND matricule = '$_iUserId') " ; 
		} else {
			$zSql .= " AND '$_zDateDebut' BETWEEN gcap_dateDebut AND gcap_dateFin AND matricule = '$_iUserId' " ; 
		}

		$zSql .= " AND matricule IN ($_iUserId)  " ; 
	...
	public function get_transaction__sqlserver($_iMatricule, $_iCompteActif, $_iUserId, $_oCandidat,$_zDateDebut, $_zDateFin,$_this,&$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1){
...
/*********************** A commenter en ligne ***********************************************/
				/*$_zDateDebut = $_this->date_fr_to_en($_zDateDebut,'/','-'); 
				$_zDateFinTraitement = $_this->date_fr_to_en($_zDateFinTraitement,'/','-');*/
				$zInsert = " (time BETWEEN '$_zDateDebut'  and  '$_zDateFin')" ;
			}
			...
			$zQuerySqlServer = "SELECT COUNT(*) AS totalrows,
									   time,
									   event_point_name, 
									   pin,
									   time 
								FROM (
										...
										UNION
										SELECT *
										FROM acc_monitor_log_2020
								) as t
								WHERE (pin=". $_iMatricule .") AND ".$zInsert."
								GROUP BY pin, time,event_point_name
								ORDER BY time ";
			//echo 			$zQuerySqlServer;die;		
			$DB1 					= $this->load->database('transactions', TRUE);	
			$zQuery 				= $DB1->query($zQuerySqlServer);
			$toGetAllTransaction 	= $zQuery->result_array();
			$i = 0;
	...
	public function CheckImCarte($_iBadgeNumber,$_iNumCarte){
		...
		$zQuerySqlServer = "SELECT * FROM userinfo_".date('Y')." WHERE Badgenumber = '".(int)$_iBadgeNumber."' AND CardNo='".$_iNumCarte."'";
		$DB1 					= $this->load->database('transactions', TRUE);	
		$zQuery 				= $DB1->query($zQuerySqlServer);		
		$toGetAllTransaction 	= $zQuery->result_array();
		if (sizeof($toGetAllTransaction)>0) {
	...
	public function getImCarte($_iBadgeNumber){
		...
		$zQuerySqlServer = "SELECT CardNo FROM userinfo_".date('Y')." WHERE Badgenumber = '".(int)$_iBadgeNumber."'";
		$DB1 					= $this->load->database('transactions', TRUE);	
		$zQuery 				= $DB1->query($zQuerySqlServer);		
		$toGetAllTransaction 	= $zQuery->result_array();
		$zCheckIm = '';

		foreach($toGetAllTransaction as $oRow){
	...
	public function getAllUserSubordonnees_last ($_oUser,$_oCandidat,$_iUserId, $_iCompteActif, &$zNotIn="",$_isPointage=TRUE) {
	...
	public function getAllUserSubordonnees ($_oUser,$_oCandidat,$_iUserId, $_iCompteActif, &$zNotIn="",$iAffiche=0) {
		...
		$zSql  = "SELECT SUBSTRING(REPLACE(cin,' ',''), 4, 9) cin1,IF(STRCMP(matricule,'ECD'),matricule,SUBSTRING(REPLACE(cin,' ',''), 4, 9)) AS matricule,c.nom as nom,c.prenom as prenom, (SELECT sigle_departement FROM $zDatabaseOrigin.departement WHERE id = c.departement) as sigle_departement,(SELECT sigle_direction FROM $zDatabaseOrigin.direction WHERE id = c.direction) as sigle_direction, (SELECT sigle_service FROM $zDatabaseOrigin.service WHERE id = c.service) as sigle_service,b.path,c.user_id FROM $zDatabaseOrigin.candidat c 
		INNER JOIN rohi.t_structure b on c.structureId = b.child_id
		WHERE 1" ;
	...
	public function getAllUserSubordonneesJustUserId ($_oUser,$_oCandidat,$_iUserId, $_iCompteActif, &$zNotIn="",$iAffiche=0) {
...
public function getGcapRecap($_iUserId){
		
		global $db;
		$zDatabaseOrigin =  $db['default']['database'] ; 

		
		$zSql = " SELECT typeGcap_libelle,gcap_typeGcapId,
				  SUM(CASE 
					WHEN gcap_dateFin = gcap_dateDebut
					THEN 
						CASE WHEN gcap_demiJournee=0 THEN '1' ELSE '0.5' END
					ELSE 
						 CASE WHEN gcap_demiJournee=0 THEN
						DATEDIFF(gcap_dateFin, gcap_dateDebut)+1
						 ELSE
						DATEDIFF(gcap_dateFin, gcap_dateDebut)+0.5
						 END
				END) AS nombre
				FROM gcap.gcap
				INNER JOIN rohi.candidat ON gcap_userSendId = user_id
				INNER JOIN gcap.`typegcap` ON typeGcap_id = gcap_typeGcapId
				WHERE gcap_userSendId = ".$_iUserId." ";

		if (isset($_REQUEST["date"]) && $_REQUEST["date"] != "") {
			$zSql .= " AND YEAR(gcap_dateDebut) = '" . $_REQUEST["date"]."'" ;
		}

		if (isset($_REQUEST["iDateSerach"]) && $_REQUEST["iDateSerach"] != "") {
			$zSql .= " AND YEAR(gcap_dateDebut) = '" . $_REQUEST["iDateSerach"]."'" ;
		}
		
				
		$zSql.= " GROUP BY gcap_typeGcapId
				  ORDER BY gcap_typeGcapId ";
	...
	public function getCompteAllGcap ($zUserId, &$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1){
		...
		$zSql = "SELECT SQL_CALC_FOUND_ROWS *
				...
				INNER JOIN rohi.t_structure b on c.structureId = b.child_id
				INNER JOIN rohi.t_sites s on s.site_id = b.site_id
		...
		if (empty($_REQUEST["iDateSerach"]) || $_REQUEST["iDateSerach"] == "") {
			...
			if (sizeof($oRequest)>0){
				...
				if (isset($toColumns[$oRequest['order'][0]['column']]) && isset($oRequest['order'][0]['dir'])){
					...
					$zSql.=" ORDER BY c.matricule DESC";
				}
	...
	public function getUserLocalisation ($_zTableName="division", $_oCandidat,$iResultSearch, $_zDateDebut, $_zDateFin, $_this, &$zDiffSearch) {
		...
		$zSql = "SELECT IF(STRCMP(matricule,'ECD'),matricule,SUBSTRING(REPLACE(cin,' ',''), 4, 9)) AS matricule FROM $zDatabaseOrigin.candidat WHERE user_id  IN (SELECT user_id  FROM $zDatabaseOrigin.candidat WHERE structureId = ".$_oCandidat[0]->structureId.")  ";
	...
	public function getCongePeriodeByMatricule ($_zUserId, $_zDateDebut, $_zDateFin,$_this){
		...
		if ($_zDateDebut != $_zDateFin){
			$zQuerySqlServer = "  SELECT g.gcap_dateDebut, g.gcap_dateFin, IF(STRCMP(matricule,'ECD'),matricule,SUBSTRING(REPLACE(cin,' ',''), 4, 9)) AS cmatricule,
									CONCAT ('', 
									...
									,'--',c.matricule,'--','O;') AS CONGE" ; 

		} else {

			$zQuerySqlServer = "  SELECT g.gcap_dateDebut, g.gcap_dateFin, IF(STRCMP(matricule,'ECD'),matricule,SUBSTRING(REPLACE(cin,' ',''), 4, 9)) AS cmatricule,
									CONCAT( gcap_dateDebut, 
									(
									CASE 
										WHEN (gcap_demiJournee = 1 AND gcap_MatinSoir = 1)
											THEN ' 08:00:00.000'
		...
		$zQuerySqlServer .= " FROM gcap.gcap g  INNER JOIN rohi.candidat c ON c.user_id = g.gcap_userSendId WHERE 1=1  ";



		/*if ($_zDateDebut != $_zDateFin) {
			$zQuerySqlServer .= " AND ('$_zDateDebut' BETWEEN gcap_dateDebut AND gcap_dateFin) " ; 
			$zQuerySqlServer .= " AND ('$_zDateFin' BETWEEN gcap_dateDebut AND gcap_dateFin ) " ; 
		} else {
			$zQuerySqlServer .= " AND '$_zDateDebut' = gcap_dateDebut AND '$_zDateFin' = gcap_dateFin " ; 
		}*/

		$zQuerySqlServer .= " AND ('$_zDateDebut' BETWEEN gcap_dateDebut AND gcap_dateFin) " ; 
		$zQuerySqlServer .= " AND ('$_zDateFin' BETWEEN gcap_dateDebut AND gcap_dateFin ) " ; 

		$zQuerySqlServer .= "  HAVING  cmatricule IN (".$_zUserId.")";
		...
		foreach ($toRow as $oRow){
			...
			$toSeparateur = explode(";", $oRow["CONGE"]);
	...
	function TempsDeTravailDesAgentsmysql(&$zDiffSearch, $_toCandidatUser, $_oCandidat, $iResultSearch, $_zDateDebut, $_zDateFin, $_this, $iCompteTotal,$iReturn=0,&$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1) {
		...
		$zQuerySqlServer = " SELECT time,
									pin,
									event_point_name as terminal,
									state,
									CASE 
										WHEN SUBSTRING(event_point_name, 1, 1) = 'P' AND state='1'
											THEN 'O' 
...
/*********************** A commenter en ligne ***********************************************/
			...
			if ($_zDateDebut !=""){
				//$toDateAccess = explode("/", $_zDateDebut);
				$zDateAccess = $_zDateDebut;
			} else {
				//$toDateAccess = explode("/", date("d/m/Y"));
				$zDateAccess = date("Y-m-d");
			}
			...
			/*$zQuerySqlServer = " SELECT time,pin,event_point_name as terminal,state, CAST( CASE 
								 ...
								 FROM [transactions].[acc_monitor_log_".$toDateAccess[2]."]
								 WHERE (pin IN ('".$zInMatriculeUser."')) AND ".$zInsert."
								GROUP BY time,event_point_name,pin,state ORDER BY time";*/

		   $iDate = date("Y", strtotime($zDateAccess));

			
			$zQuerySqlServer = " SELECT time,pin,event_point_name as terminal,state, 
								(CASE
									WHEN SUBSTRING(event_point_name, 1, 1) = 'P' AND state='1' THEN 'O'
									WHEN SUBSTRING(event_point_name, 1, 1) = 'P' AND state='0' THEN 'I'
									WHEN SUBSTRING(event_point_name, 1, 1) = 'S' THEN 'O'
									WHEN SUBSTRING(event_point_name, 1, 1) = 'E' THEN 'I' 
								 END) AS CHECKTYPE
								 FROM acc_monitor_log_".$iDate."
								 WHERE (pin IN (".$zInMatriculeUser.")) AND ".$zInsert."
								GROUP BY time,event_point_name,pin,state ORDER BY time";
			...
			/*$zQuerySqlServer = " SELECT TIME,pin,event_point_name AS terminal,state,
								...
								FROM  transactions.acc_monitor_log_2021
								...
								AND 
								(TIME BETWEEN '2021-01-10 00:00:00.000' AND '2021-01-10 23:00:00.000')  GROUP BY TIME,event_point_name,pin,state ORDER BY time";*/
			...
			foreach ($toResult as $oResult){
				...
				$oArray = array();
				$oArray['time']			= $oResult['time'];
				$oArray['pin']			= $oResult['pin'];
				$oArray['CHECKTYPE']	= $oResult['CHECKTYPE'];
...
/*********************************************** A decommenter en ligne ***********************************************************/
	...
	public function setPdf ($_toListe, $_zDateDebut, $_zDateFin) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			$oPdf->SetX(50);
			$oPdf->Cell(75,7,utf8_decode('Nom et Prénom'),1,0,'L',1);
			...
			$oPdf->SetX(125);
			$oPdf->Cell(60,7,'Localisation',1,0,'C',1);

			$oPdf->SetX(185);
			$oPdf->Cell(25,7,'Nbr de jour',1,0,'C',1);

			$oPdf->SetX(210);
			$oPdf->Cell(30,7,utf8_decode('Heures Règl.'),1,0,'C',1);

			$oPdf->SetX(240);
			$oPdf->Cell(25,7,utf8_decode('Heures Sup.'),1,0,'C',1);

			$oPdf->SetX(265);
			$oPdf->Cell(25,7,utf8_decode('Heures Total'),1,0,'C',1);
			...
			foreach ($_toListe as $oListe) {
				...
				$oPdf->SetX(50);
				$oPdf->Cell(75,7,utf8_decode($oListe->data['nom']) . " " . utf8_decode($oListe->data['prenom']),$zBorder,0,'C',1);

				$oPdf->SetX(125);
				$oPdf->Cell(60,7,utf8_decode($oListe->data['path']),$zBorder,0,'C',1);
				...
				$oPdf->SetX(240);
				$oPdf->Cell(25,7,$oListe->Tot,$zBorder,0,'C',1);

				$oPdf->SetX(265);
				$oPdf->Cell(25,7,$oListe->iTotTravailler,$zBorder,0,'C',1);
	...
	public function setExcel ($_toListe, $_zDateDebut, $_zDateFin) {
		...
		foreach ($_toListe as $oListe) {
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $iRowDynamic, $oListe->data['path']);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $iRowDynamic, $oListe->iDiffDate);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $iRowDynamic, $oListe->value);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $iRowDynamic, $oListe->Tot);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(8, $iRowDynamic, $oListe->iTotTravailler);
			$iRowDynamic++;
	...
	public function setExcelCompteGcap ($_toListe, $_iDate) {
		...
		$default_style_ligne3 = array(
			...
			'fill' => array(
					'type' => PHPExcel_Style_Fill::FILL_SOLID,
					'color' => array('rgb' => '3B76D0')
			 )
			 
		);

		$default_style_ligne4 = array(
			'font' => array(
				'name' => 'Verdana',
				'color' => array('rgb' => '000000'),
				'size' => 10
			...
			'fill' => array(
					'type' => PHPExcel_Style_Fill::FILL_SOLID,
					'color' => array('rgb' => '20A536')
			 )
			 
		);

		$default_style_ligne5 = array(
			'font' => array(
				'name' => 'Verdana',
				'color' => array('rgb' => '000000'),
				'size' => 10
			...
			'fill' => array(
					'type' => PHPExcel_Style_Fill::FILL_SOLID,
					'color' => array('rgb' => 'D7BE1A')
			 )
			 
		);

		$default_style_ligne6 = array(
			'font' => array(
				'name' => 'Verdana',
				'color' => array('rgb' => '000000'),
				'size' => 10
			...
			'fill' => array(
					'type' => PHPExcel_Style_Fill::FILL_SOLID,
					'color' => array('rgb' => 'C41AD7')
			 )
			 
		);

		$default_style_ligne7 = array(
			'font' => array(
				'name' => 'Verdana',
				'color' => array('rgb' => '000000'),
				'size' => 10
			...
			'fill' => array(
					'type' => PHPExcel_Style_Fill::FILL_SOLID,
					'color' => array('rgb' => 'D78B1A')
			 )
			 
		);

		$default_style_ligne8 = array(
			'font' => array(
				'name' => 'Verdana',
				'color' => array('rgb' => '000000'),
				'size' => 10
		...
		$tHead1 = array(						
						...
						'NBR DE JOUR'					=> 	' ANNEE ', 
						'CONGE'							=> 	' CONGE ',
						'AUTORISATION D\'ABSENCE'		=> 	' AUTORISATION D\'ABSENCE ',
						'PERMISSION'					=> 	' PERMISSION ',
						'REPOS MEDICAL'					=> 	' REPOS MEDICAL ',
						'MISSION'						=> 	' MISSION ',
						'FORMATION'						=> 	' FORMATION ',
					  );
		...
		$objPHPExcel->getActiveSheet()->getStyle('A3:L3')->applyFromArray($default_style);

		$iRow = 5 ; 
		$iCol = 0;
		foreach ($tHead1 as $zValue) {
			$objPHPExcel->getActiveSheet()->getStyle('A5:L5')->applyFromArray($default_style_ligne2);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($iCol, $iRow, utf8_encode($zValue));
			$iCol++;
		}
		...
		foreach ($_toListe as $oListe) {
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $iRowDynamic, $oListe[0]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $iRowDynamic, $oListe[1]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $iRowDynamic, $oListe[2]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $iRowDynamic, $oListe[3]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $iRowDynamic, $oListe[4]);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $iRowDynamic, $_iDate);

			$iIncrement = 6;
			$toAlhpabet = array("A", "B", "C", "D" , "E", "F", "G", "H", "I", "J","K","L","M");
			foreach ($oListe["toListeRecap"] as $oListeRecap){
				$zAffiche = $oListeRecap["nombre"] ;

				$iListe = $iIncrement + 1;

				$zStyle = $default_style_ligne2;
				switch($oListeRecap["gcap_typeGcapId"]){
					case CONGE:
						$zStyle = $default_style_ligne3;
						$iIncrement = 6;
						break;

					case AUTORISATION_ABSENCE:
						$zStyle = $default_style_ligne4;
						$iIncrement = 7;
						break;
					
					case PERMISSION:
						$iIncrement = 8;
						$zStyle = $default_style_ligne5;
						break;
					
					case REPOS_MEDICAL_LUCIA:
						$iIncrement = 9;
						$zStyle = $default_style_ligne6;
						break;

					case MISSION:
						$iIncrement = 10;
						$zStyle = $default_style_ligne7;
						break;
				...
				if($toAlhpabet[$iIncrement].$iRowDynamic != ''){
					$objPHPExcel->getActiveSheet()->getStyle($toAlhpabet[$iIncrement].$iRowDynamic)->applyFromArray($zStyle);
				}
				$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($iIncrement, $iRowDynamic, $zAffiche);
		...
		header("Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
		header("Content-Disposition: attachment; filename=recapitulation.gcap_".date("YmdHms").".xlsx");
		header("Cache-Control: max-age=0");
	...
	public function tropBe($mois,$annee){
	...
	/*	$zQuerySqlServer = " SELECT * from [ZKAccess2020].[dbo].[acc_monitor_log] 
		...
		$DB1 						= $this->load->database('gcap', TRUE);
		$zDatabaseGcap	 			= $db['gcap']['database'] ;
		$select_candidat			= "SELECT * FROM rohi.candidat where sanction IN ('00','34','40')  ";
		
		$zQuery 					= $this->db->query($select_candidat);
		$toRow 						= $zQuery->result_array();
		$zQuery->free_result();
		$iIndex		=	1 ;
		foreach ($toRow as $oRow) {		
			$delete_rows_transactions	=	" DELETE FROM Transactions.acc_monitor_log_".$annee." WHERE pin='".$oRow["matricule"]."' AND  YEAR(time) ='".$annee."' AND MONTH(time)='".$mois."'  " ;
			$this->db->query($delete_rows_transactions);
				
			$zQuerySqlServer = " SELECT * from [ZKAccess".$annee."].[dbo].[acc_monitor_log]
			WHERE  pin='".$oRow["matricule"]."' AND  YEAR(time) ='".$annee."' AND MONTH(time)='".$mois."'  ";
			$oResult = $this->executeQuery($zQuerySqlServer);
			$toArray		=	 array();
			while($oArrayResult = odbc_fetch_array($oResult)){
				$oArray = array();
				$oArray['time']				= odbc_result($oResult,10); 
				$oArray['pin']				= odbc_result($oResult,11); 
				$oArray['event_point_name']	= odbc_result($oResult,15); 
				$statut						=  substr($oArray['event_point_name'],0,1);
				$insert_rows_transactions	= "INSERT INTO Transactions.acc_monitor_log_".$annee." (time,pin,state,event_point_name) VALUES ('".$oArray['time']."','".$oArray['pin']."','".$statut."','".$oArray['event_point_name']."')";
				$this->db->query($insert_rows_transactions);
			}
