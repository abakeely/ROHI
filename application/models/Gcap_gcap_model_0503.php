...
class Gcap_gcap_model extends CI_Model {
	...
	function getAllFichePoste($iDepartement=0){
		...
		$zSql= " SELECT direction.libele as direction,departement.libele as departement,candidat.nom,candidat.prenom,candidat.user_id,fichePoste_id, fichePoste_userId,fichePoste_intitule,
				...
				INNER JOIN $zDatabaseGcap.exigenceposte ON exigencePoste_fichePosteId = fichePoste_id WHERE candidat.user_id = 617  GROUP BY candidat.user_id ORDER BY direction.libele ASC ";
...
/****************************************** Fiche de poste **************************************************/
		...
		switch ($_iCompteActif)
		{
			...
			case COMPTE_RESPONSABLE_PERSONNEL :
				...
				/*if ($_oUser['im'] == '350210' ||  $_oUser['im'] == '260011') {
					...
					$zSql = "SELECT user_id FROM $zDatabaseOrigin.candidat c12 INNER JOIN $zDatabaseOrigin.departement s ON c12.departement = s.id
						WHERE s.id  = (SELECT departement FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat[0]->id.") AND c12.region_id = ".$_oCandidat[0]->region_id." 
						" ;
				}*/
			...
			case COMPTE_AUTORITE :
				...
				if ($_oUser['serv'] != 0) {
					...
					if($_oCandidat[0]->departement == '8'){
						$zSql = "SELECT user_id FROM $zDatabaseOrigin.candidat c12 INNER JOIN $zDatabaseOrigin.departement s ON c12.departement = s.id
							WHERE s.id  = (SELECT departement FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat[0]->id.") AND c12.region_id = ".$_oCandidat[0]->region_id." 
							" ;
					} else {
					
						if ($_oCandidat[0]->service < '700' && $_oCandidat[0]->service != '') {
			...
			case COMPTE_ADMIN :
				$zUserId = "" ; 
				$zNotIn = " AND (sanction='0' || sanction='' || sanction='00' || sanction IS NULL)"; 
				break;
		}
	...
	public function getAllChefHierarchique ($_this,$_oUser,$_oCandidat,$_iUserId, &$zNotIn="",$iAffiche=0,$iIsChefService=0) {
		...
		if ($iEvaluateuruserId > 0){
			...
			INNER JOIN userCompte ON userCompte_userId = user_id AND userCompte_compteId = ".COMPTE_AUTORITE."
			WHERE d.id  = (SELECT direction FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat['id'].") AND c12.region_id = ".$_oCandidat['region_id']."  AND (fonction_actuel LIKE '%directeur%' OR poste LIKE '%directeur%')";
			...
			if($_oCandidat['departement'] == '8'){
				...
				if ($_oCandidat['service'] < '700' && $_oCandidat['service'] != '') {
					...
					$zSql = "SELECT user_id FROM $zDatabaseOrigin.candidat c12 INNER JOIN $zDatabaseOrigin.service s ON c12.service = s.id
						WHERE s.id  = (SELECT service FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat['id'].") AND c12.region_id = ".$_oCandidat['region_id']." 
						" ;
					
				} else {
					/* même direction */
					$zSql = "SELECT user_id FROM $zDatabaseOrigin.candidat c12 INNER JOIN $zDatabaseOrigin.direction d ON c12.direction = d.id
						WHERE d.id  = (SELECT direction FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat['id'].") AND c12.region_id = ".$_oCandidat['region_id']." 
						" ;
				}
			}
	...
	public function get_all_gcap_extrants($_zCandidat,$_oUser,$_oCandidat, $_iUserId, $_iCompteActif,&$_iNbrTotal = 0){
		...
		$zSql= "SELECT SQL_CALC_FOUND_ROWS *,gcap_id as iGcapId,CONCAT(c.nom,' ',c.prenom) as agent,15 as testtt FROM gcap
				INNER JOIN type ON gcap_typeId = type_id
	...
	public function get_all_gantt_planning($oUser,$_oDataSearch=array(), $_oCandidat, $_iUserId, $_iTypeGcapId,$_iCompteActif, &$_iNbrTotal = 0, $_iValPerPage = NB_PER_PAGE, $_iCurrPage = 1, $_zSortSens = "DESC", $_zFieldOrder = "gcap_id"){
		...
		$zSqlInsert = "";
		/*if(isset($_oDataSearch['month']) && $_oDataSearch['month']>0){
			$zSqlInsert .= " AND (month(gcap_dateFin) < ". $_oDataSearch['last']." AND month(gcap_dateDebut) >= ".$_oDataSearch['monthPrev'].") ";
		}
		if(isset($_oDataSearch['year']) && $_oDataSearch['year']>0){
			$zSqlInsert .= " AND (year(gcap_dateFin) = ". $_oDataSearch['last']." or year(gcap_dateDebut) = ".$_oDataSearch['year'].") ";
		}*/
		...
		$zSql= "select c.nom,c.prenom,c.user_id as iUserId,
		(SELECT GROUP_CONCAT(CONCAT(gcap_dateDebut,'/',gcap_dateFin,'/',gcap_typeGcapId,'/',gcap_demiJournee,'/',gcap_MatinSoir) SEPARATOR ';') FROM gcap WHERE gcap_userSendId = iUserId ".$zSqlInsert." AND (gcap_valide IS NULL OR gcap_valide = 1)) as GcapDate 
		FROM $zDatabaseOrigin.candidat c WHERE user_id IN (".$zUserId.") and candidat_economie IS NULL";
	...
	public function get_Organisation($id = FALSE, $_zNomOrganisation='departement', $_iTypeSearch = 0){
		...
		switch ($_iTypeSearch){
			...
			case 1:
				$zSql .= " where departement_id IN (0,".$id.") ORDER BY departement_id,id ";
				$query = $this->db->query($zSql);
				return $query->result_array();
				break;

			case 2:
				$zSql .= " where direction_id IN (0,".$id.") ORDER BY direction_id,id ";
				$query = $this->db->query($zSql);
				return $query->result_array();
				break;
	...
	public function get_sigle($_iUserId=false, $_isSigle=1){
	...
	public function get_all_list_institution($zTerm= "-111"){
		...
		$zSql= "select * from $zDatabaseOrigin.institution WHERE (institution_libelle LIKE '%$zTerm%')";
		...
		switch ($_iSessionCompte)
		{
			...
			case COMPTE_RESPONSABLE_PERSONNEL :
				...
				if ($_oUser['serv']!= '') {
						...
						if ($_oUser['reg'] > 0){
							$zUserId .= " AND c.region_id = ".$_oUser['reg']; 
						}
						...
						if ($_oCandidat[0]->service < '700') {
					
							/* même service */
							$zUserId = "SELECT user_id FROM $zDatabaseOrigin.candidat c INNER JOIN $zDatabaseOrigin.service s ON c.service = s.id
								WHERE s.id  = (SELECT service FROM $zDatabaseOrigin.candidat WHERE id = ".$_oCandidat[0]->id.") AND c.region_id = ".$_oCandidat[0]->region_id." 
								AND c.user_id <> $_iUserId " ;
	...
	public function setImprimerPdf ($_oData, $_this) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			$oPdf->Cell($iRight1,$iInterline,'  MINISTÈRE DE L\'ECONOMIE DES FINANCES',"L",0,'C',1);
			...
			foreach ($_oData['toSigle'] as $oSigle) {
				...
				$zDepartement = $oSigle['departementNom']; 
				$iCountDepartement = str_word_count($zDepartement);

				$iMargin = 30;
				if ($iCountDepartement > 10){
					$iMargin = 60;
				}
				
				$oPdf->Cell($iRight1,$iInterline,$oSigle['departementNom'],"L",0,'C',1);
				$oPdf->Cell($iRight2,$iInterline,'',0,0,'C',1);
				...
				if ($zCongeSuite !=""){
					...
					if($oSigle['directionNom'] != '' && ($oSigle['directionNom'] != $oSigle['departementNom'])){
						$iIncrement += 6 ; 
						$oPdf->SetY($iIncrement);
						$oPdf->Cell($iLeft,$iInterline,'DEMANDE DE :                 ',0,0,'R',1);
						$oPdf->Cell($iRight1,$iInterline,$oSigle['directionNom'],"L",0,'C',1);
						...
						$oPdf->Cell($iRight1+$iMargin,$iInterline,$oSigle['departementNom'],"L",0,'C',1);
					...
					if($oSigle['directionNom'] != '' && ($oSigle['directionNom'] != $oSigle['departementNom'])){
						$iIncrement += 6 ; 
						$oPdf->SetY($iIncrement);
						$oPdf->Cell($iLeft,$iInterline,'DEMANDE '.$_oData['zTitre'].'				',0,0,'R',1);
						$oPdf->Cell($iRight1,$iInterline,$oSigle['directionNom'],"L",0,'C',1);
				...
				if ($zCongeSuite !=""){
					if($oSigle['serviceNom'] != '' && ($oSigle['serviceNom'] != $oSigle['directionNom'])){
						...
						$iIncrement += 6 ; 
						$oPdf->SetY($iIncrement);
						$oPdf->Cell($iLeft,$iInterline,'   CONGE',0,0,'C',1);
						$oPdf->Image(base_url().'assets/gcap/images/check.jpg',62,51.5,4);
						$oPdf->Cell($iRight1,$iInterline,$oSigle['serviceNom'],"L",0,'C',1);
						//$oPdf->Cell($iRight2,$iInterline, $_oData['zTitre2'],0,0,'C',1);

						$iIncrement += 6 ; 
						$oPdf->SetY($iIncrement);
						$oPdf->Cell($iLeft,$iInterline,'  AUTORISATION D\'ABSENCE                         ',0,0,'R',1);
						$oPdf->Image(base_url().'assets/gcap/images/check.jpg',62,57.5,4);
						$oPdf->Cell($iRight1,$iInterline," ","L",0,'R',1);
						...
						if ($oSigle['sigle_service'] == "DRHA") {
							...
							$oPdf->Cell($iLeft,$iInterline,'   CONGE',0,0,'C',1);
							$oPdf->Image(base_url().'assets/gcap/images/check.jpg',62,51.5,4);
							$oPdf->Cell($iRight1,$iInterline,"Service de la Gestion des Ressources Humaines","L",0,'C',1);
							//$oPdf->Cell($iRight2,$iInterline,$_oData['zTitre2'],0,0,'C',1);
							...
							$oPdf->Image(base_url().'assets/gcap/images/check.jpg',10,10,5);
							$iIncrement += 6 ; 
							$oPdf->SetY($iIncrement);
							$oPdf->Cell($iLeft,$iInterline,'   CONGE',0,0,'C',1);
			...
			$oPdf->Cell($iLeft,$iInterline,'',0,0,'C',1);
			$oPdf->Cell($iRight1,$iInterline,'',"L",0,'C',1);
			$oPdf->Cell($iRight2,$iInterline,"EN " . $_oData['zTitre2'],0,0,'C',1);
			$oPdf->Ln();*/
			...
			if ($zCongeSuite !=""){
				...
				$oPdf->Image(base_url().'assets/gcap/images/check1.jpg',$iLeft+115,$iInterline+63.5,4);
				...
				$oPdf->Cell($iLeft,$iInterline,'',0,0,'C',1);
				$oPdf->Cell($iRight1+10,$iInterline,'',"L",0,'C',1);
				$oPdf->Image(base_url().'assets/gcap/images/check1.jpg',$iLeft+115,$iInterline+69.5,4);
			...
			$oPdf->SetFont('Arial','');
			$oPdf->Cell($iLeft-18,$iInterline,utf8_decode($oCandidat["poste"]),0,0,'L',1);
			/**********************************************************************************/

			$oPdf->SetFont('Arial','B');
			$oPdf->Cell(18,$iInterline,'		Fonction : ',"L",0,'L',1);
			$oPdf->SetFont('Arial','');
			$oPdf->Cell($iRight-18,$iInterline,utf8_decode($oCandidat["poste"]),0,0,'L',1);
			...
			/*if ($oGcap['conv_pers'] != '') {
				...
				$zAdresse = utf8_decode($oGcap['conv_pers']);
				$zAdresse1 = wordwrap($zAdresse, 60, "-------");

				$iNombreLigne =  substr_count($zAdresse1, '-------', strpos($zAdresse1, '.') ); // 1
				$iNombreLigne = ($iNombreLigne>1)?$iNombreLigne:1;

				$toLigne = explode("-------", $zAdresse1);
				...
				$oPdf->SetFont('Arial','');
				$oPdf->Cell($iLeft-46,$iInterline,$toLigne[0],0,0,'L',1);
				...
				$oPdf->SetFont('Arial','');
				$oPdf->Cell($iRight-46,$iInterline,$toLigne[0],0,0,'L',1);

				if(isset($toLigne[1])){

					$zAfficheReste = str_replace("-------","",$toLigne[1]);
					$iIncrement += 6 ; 
					$oPdf->SetXY(12,$iIncrement);
					$oPdf->Cell(123,$iInterline,$zAfficheReste,"R",0,'L',1);

					$oPdf->SetXY(137,$iIncrement);
					$oPdf->Cell(100,$iInterline,$zAfficheReste,0,1,'L',1);
				}

				//$iIncrement += 6 ; 
				//$oPdf->SetXY(12,$iIncrement);
				//$oPdf->Cell(123,$iInterline,$toLigne[0],"R",0,'L',1);

				//$oPdf->SetXY(137,$iIncrement);
				//$oPdf->Cell(100,$iInterline,$toLigne[0],0,1,'L',1);

				//if(isset($toLigne[1])){
				//	$iIncrement += 6 ; 
				//	$oPdf->SetXY(12,$iIncrement);
				//	$oPdf->Cell(123,$iInterline,$toLigne[1],"R",0,'L',1);

				//	$oPdf->SetXY(137,$iIncrement);
				//	$oPdf->Cell(100,$iInterline,$toLigne[1],0,1,'L',1);
				//}
			}*/
			...
			if ($oGcap['conv_pers'] != '') {
				...
				/*$iIncrement += 6 ; 
				$oPdf->SetY($iIncrement);
				$oPdf->SetLeftMargin(12);
				$oPdf->MultiCell($iLeft-2,$iInterline,'		'.utf8_decode($oGcap['conv_pers']),'R','L',true);
				/**********************************************************************************/
				/*$oPdf->SetY($iTempIncrement);
				$oPdf->SetLeftMargin(135);*/
				$oPdf->SetFont('Arial','B');
				$oPdf->Cell(46,$iInterline,'		Convenances personnelles : ',"L",0,'L',1);
				$oPdf->SetFont('Arial','');
				/*$iTempIncrement += 6 ; 
			...
			foreach ($_oData['toDataListeFraction'] as $oListe) {
				...
				$iReste = $this->getDecisonReste($oCandidat["user_id"],$oListe['decision_id']);
			...
			$oPdf->SetY($iIncrement);
			$oPdf->Cell($iLeft-50,$iInterline,"		L'intéressé".$zSuffixe."",0,0,'L',1);
			/**********************************************************************************/
			...
			$oPdf->Image($zImageLogoUrl,$iLeft+15,5,22.5);

			if ($zLogo != "") {
				$zImageLogoUrl = base_url().'assets/upload/logo/' . $zLogo;
				$oPdf->Image($zImageLogoUrl,$iLeft+140,5,20);
			}
	...
	public function getDecisonReste($_iUserId, $_iDecisionId){
		...
		$zSql  = "select (decision_nbrJour -(IFNULL((SELECT SUM(fraction_nbrJour) FROM fraction WHERE fraction_decisionId = decision_id AND fraction_userId = '$_iUserId'),0))) AS reste
				FROM decision INNER JOIN type ON decision_typeId = type_id where decision_userId = '$_iUserId' AND decision_finalisation = 1 AND decision_dateFinalisation <> '' " ; 
	...
	public function __setImprimerDecisionPdf ($_oData, $_this) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			$oPdf->SetY($iIncrement);
			$oPdf->SetFont('Arial','B');
			$oPdf->Cell($iLeft,$iInterline,'  MINISTÈRE DE L\'ECONOMIE DES FINANCES',0,0,'C',1);
	...
	public function setImprimerEtat ($_oData, $_this) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			foreach ($_oData['toSigle'] as $oSigle) {
				...
				$zDecisionLong = $zTitreUser . " " .utf8_decode($oCandidat[0]->nom)." " . utf8_decode($oCandidat[0]->prenom) . ". IM ".$oCandidat[0]->matricule.", ".utf8_decode($oCandidat[0]->poste).". ".utf8_decode($zService)." du Ministère de l'économie et des Finances.";
	...
	public function setImprimerDecisionPdf ($_oData, $_this, $_iId) {
		...
		require(APPLICATION_PATH ."pdf/fpdf/fpdf.php");
			...
			$zString = " En service auprès de la " . $zService . " Ministère de l'économie et des Finances." ; 
			
			$zStringPart1 = $zString;
			$zStringPart2 = '';
			if(strlen($zString) > 53){
				$zStringPart1 = substr($zString,0,73);
				if(strlen($zString) > 120) 
					$zStringPart2 = substr($zString,73,90).'...';
				else $zStringPart2 = substr($zString,73,strlen($zString));
			...
			$oPdf->SetX(70);
			$oPdf->SetFont('Times','',11);
			$oPdf->Cell(130,7,$zStringPart2,'LR',0,'L',1);
