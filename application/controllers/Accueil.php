	...
	public function communique($_iCategorieId=0){
		...
		if (empty($candidat)){
			...
			if($iRet == 1){	
				...
				$iAfficheMinistre = 0;
				if(empty($_SESSION['iAfficheMinistre'])){
					$_SESSION['iAfficheMinistre'] = 1;
					$iAfficheMinistre = 1;
				}
				...
				$oData['iSuc']				= $iSuc ;
				$oData['iAfficheMinistre']	= $iAfficheMinistre ;
				...
				switch ($_zHashUrl) {
					...
					case 'ajax':
						...
						foreach ($toGetListe as $oGetListe){
							...
							$zAction = $zAction . '&nbsp;&nbsp;'. '<a href="'.base_url().'cv/fpdf_cv/'.$oGetListe->id.'" target="_blank" title="Imprimer CV" alt="Imprimer CV"><i style="font-size:22px;" class="la la-print"></i></a>' ; 
