	...
	* @subpackage documentation */ 
		...
		if($iRet == 1){	
			...
			foreach ( $parameters as $key =>$value ){
				if( is_integer($key) ){
					...
					$first_comments		=	$this->postGetValue ("first_comments");
					$second_comments	=	$this->postGetValue ("second_comments");
					$this->QuestionnaireService->updateQuestion($oUser["id"],$porte,$fonction,$structureId,$lieu_service,$first_comments,$second_comments);
					
					for ( $iIndex = 1 ; $iIndex < 29 ; $iIndex ++ ){
						$commentaire	=	$_POST["answer_comments_".$iIndex ];
						$this->QuestionnaireService->updateCommentaire($oUser["id"],$iIndex,$commentaire);
					}
				}
			}
			//prdie;
			redirect("Questionnaire/questionnaireDgfag");
		}
	}
	...
	public function ajaxTelechargerFiche($_user_id){
		...
		if($iRet == 1){	
			...
			$structure		=	$this->QuestionnaireService->getStructure($_user_id);
			$candidat		=	$this->CandidatService->get_by_user_id($_user_id);
			$zFonction		=	$this->QuestionnaireService->getFonction($_user_id)["quizz_referentiel_libelle_fr"];
			$zLieuDeTravail	=	$structure["site_libelle"];
			$zDirection		=	"";
			$zService		=	"";
			$zRegion		=	$structure["region_libelle"];
			$zDistrict		=	$structure["district_libelle"];
			if(!$zRegion){
				$zRegion	=	"Analamanga";
			}
			if(!$zDistrict){
			...
			$textrun->addTextBreak();
			$textrun->addText('I.3 Direction :', ['bold'=>true]);
			$textrun->addTextBreak();
			$textrun->addText('I.4 Service :', ['bold'=>true]);
			$textrun->addTextBreak();
			...
			$zPremierCommentaire		=	$this->QuestionnaireService->getComments($_user_id)["first_comments"];
			...
			$zDeuxiemeCommentaire		=	$this->QuestionnaireService->getComments($_user_id)["second_comments"];
	...
	public function do_get_sites(){
		$toDatas				=	array() ;
		$child_id				=	$this->postGetValue ("child_id","");
		$sites					=	$this->QuestionnaireService->do_get_sites($child_id);
