	...
	* @subpackage documentation */ 
	...
	public function questionnaireDgfag($_zHashModule = FALSE, $_zHashUrl = FALSE){
		...
		if($iRet == 1){	
			...
			if( $rangQuestion["rang_question"] == 64 ){
				$oData['enquete_effectue']	= "1" ;
			}else{
				$oData['enquete_effectue']	= "0" ;
			}
			$toQuestions					=	$this->QuestionnaireService->getNextQuestion("1",$oUser["id"]);
			$oData['toQuestions']			=	$toQuestions ; 
			$iCompteActif = $this->getSessionCompte() ;
			/*if ($iCompteActif == COMPTE_AGENT){
				$this->load_my_view_Common('questionnaire/questionnaire_dgfag_agent.tpl',$oData, $iModuleId);	
			}
			if ($iCompteActif == COMPTE_EVALUATEUR){
				$this->load_my_view_Common('questionnaire/questionnaire_dgfag_eval.tpl',$oData, $iModuleId);	
			}
			if ($iCompteActif == COMPTE_AUTORITE){
				$this->load_my_view_Common('questionnaire/questionnaire_dgfag_autorite.tpl',$oData, $iModuleId);	
			}*/
			$this->load_my_view_Common('questionnaire/index.tpl',$oData, $iModuleId);	
