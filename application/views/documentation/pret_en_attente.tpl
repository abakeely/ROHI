					...
					<div class="box">
						<div class="card-body">
							...
							<div class="text-center" >
								...
								<div align="center">
									<a href="{$zBasePath}documentation/pret_en_pret" class="btn">LISTE DES EMPRUNTS</a>
									<a href="{$zBasePath}documentation/pret_en_attente" class="btn">LISTE D'ATTENTE</a>
								</div><br>
							</div>
							...
							<div style="width: 100%; height: 350px; overflow-y: scroll;">
								<table class="table table-striped table-bordered table-hover" id="table_liste_pretagent">
									...
									<tbody>
											...
											{foreach from=$oData.list_pret item=pret_en_attente}	
											<tr>
												<td>{$ordre}</td>
												<td>{$pret_en_attente->date_reservation}</td>
												{if $user['role'] == "biblio"}
												<td>{$pret_en_attente->candidat?$pret_en_attente->candidat->matricule.'-'.$pret_en_attente->candidat->nom.' '.$pret_en_attente->candidat->prenom:''}</td>
												{/if}
												<td>{$pret_en_attente->livre->theme_livre.libele}</td>
												<td id="cote_{$pret_en_attente->id}">{$pret_en_attente->livre->cote_livre}</td>
												<td id="title_{$pret_en_attente->id}">{$pret_en_attente->livre->titre_livre}</td>
									
												<td >{$pret_en_attente->livre->auteur_livre.libele}</td>
												<td>{$pret_en_attente->livre->edition_livre}</td>
												{if $user['role'] != "biblio"}
												<td>{$pret_en_attente->livre->lieu_livre['libele']}</td>
												{/if}
												<td>{$pret_en_attente->livre->langue_livre.libele}</td>
												<td>{$pret_en_attente->date_validation}</td>
												<td>{$pret_en_attente->date_retour}</td>
												{if $user['role'] == "biblio"}
												<td>{$pret_en_attente->date_retour2}</td>
												{/if}
												<td>
												
												{if $pret_en_attente->statut == 0}
												{if $user['role'] == "biblio"}
													...
													<img src="{$zBasePath}assets/img/img_sad/boule/en_pret_en_attente.png" alt=" En pr&ecirc;t"  class="to_click" border="0" height="38" width="38" onclick="changeStatut({$pret_en_pret->id},2)">
													...
													{else}
														...
														<img src="{$zBasePath}assets/img/img_sad/boule/en_pret_en_attente.png" alt="En pr&ecirc;t"  border="0" height="40" width="40">
														<img src="{$zBasePath}assets/img/img_sad/boule/retour2.png" alt="Rendu"  border="0" height="28" width="28">
														
													{/if}
												{/if}
												
												{if $pret_en_attente->statut == 1}
												{if $user['role'] == "biblio"}
													<img src="{$zBasePath}assets/img/img_sad/boule/disponible.png" alt="Disponible"  border="0" height="28" width="28">
												...
												{if $pret_en_attente->statut == 2}
												<img src="{$zBasePath}assets/img/img_sad/boule/en_pret_en_attente.png" title="En pr&ecirc;t"  class="to_click" border="0" height="30" onclick="changeStatut({$pret_en_pret->id},1)">
												{/if}
												{if $pret_en_attente->statut == 3}
													<img src="{$zBasePath}assets/img/img_sad/boule/retour2.png" title="Rendu"  border="0" height="28">
												{/if}
												{if $pret_en_attente->statut == 4}
													<img src="{$zBasePath}assets/img/img_sad/boule/email.png" title="Livre envoie par email" border="0" height="28">
												{/if}
											</td>
											{if $user['role'] == "biblio"}
											<td><a href="{$zBasePath}cv/fpdf_cv/{$pret_en_attente->candidat->id}"><img src="{$zBasePath}assets/img/img_sad/boule/cv.png" border="0" height="53" width="60"></a></td><?php }?>
											{/if}
										</tr>
