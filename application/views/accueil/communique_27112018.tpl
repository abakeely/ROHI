...
function showVideo(_zVideo){$("#dialog7").html('<video id="largeVideo" width="100%" webkit-playsinline="true" autoplay="no" loop="yes" data-weborama-videoplayer="true" preload="metadata" controls="true" style="position: center; left: 0px; top: 0px; z-index: 200;"><source type="video/mp4" src="{/literal}{$zBasePath}{literal}assets/'+_zVideo+'"></video>');$("#dialog7").dialog("open")}
$(document).ready(function()
{$("#dialog7").dialog({width:'35%',modal:!0,autoOpen:!1,dialogClass:'myPosition',close:function(){document.getElementById("largeVideo").pause()}})})
{/literal}{if sizeof($oData.toChangeLocalite)>0}{literal}
function sendLocalite(_iType){var iUserId=$("#iUserLocaliteId").val();$.ajax({url:"{/literal}{$zBasePath}{literal}critere/saveLocaliteService/",type:'Post',data:{iUserId:iUserId,iType:_iType},success:function(data,textStatus,jqXHR){var iSize=$("#iSize").val();if(iSize>1){$("#dialog6").html();$("#dialog6").dialog("close");$(".popUpLocalite").click()}else{$("#dialog6").html();$("#dialog6").dialog("close")}
event.preventDefault()},async:!1})}
{/literal}{/if}{literal}
$(document).ready(function()
{$('.malefaka').click(function(){var idActive=$(this).attr('idActive');$('.malefaka a').removeClass("active");$(this).find('a').addClass("active");$(".all").hide();$("."+idActive).show()})
$("#dialog7").dialog({width:'35%',modal:!0,autoOpen:!1,dialogClass:'myPosition'});{/literal}{if $oData.iAfficheMFBNC==1}{literal}
{/literal}{/if}{literal}
{/literal}{if sizeof($oData.toChangeLocalite)>0}{literal}
$("#dialog6").dialog({autoOpen:!1,width:'75%',closeOnEscape:!1,modal:!0,show:{effect:"slide",duration:1000},hide:{effect:"drop",duration:1000}});$("#dialog6").dialog("option","buttons",{});$(".popUpLocalite").click(function(event){$("#dialog6").html();var zTitle=$(this).attr("title");$('#dialog6').dialog('option','title',"Confirmation changement localité de service d'un agent");$.ajax({url:"{/literal}{$zBasePath}{literal}critere/getChangeLocaliteResPers/",type:'post',data:{},success:function(data,textStatus,jqXHR){$("#dialog6").html(data);$("#dialog6").dialog("open");event.preventDefault()},async:!1})});$(".popUpLocalite").click();{/literal}{/if}{literal}
{/literal}{if $oData.iPopUpId==0}{literal}
$("#dialog3").dialog({autoOpen:!1,width:'75%',closeOnEscape:!1,modal:!0,show:{effect:"slide",duration:1000},hide:{effect:"drop",duration:1000},buttons:[{text:'Valider',"class":'saveButtonClass',click:function(){var iTestValid=1;var zMessage="";var iUserId=$("#iUserId").val();var zPhone=$("#zPhone").val();var zEmail=$("#zEmail").val();var iModeForChange=$('#iModeForChange').val();var iProvinceId=$('#iOrganisation_1').val();var iRegionId=$('#iOrganisation_2').val();var iDistrictId=$('#iOrganisation_3').val();var iDepartementId=$('#iOrganisation_4').val();var iDirectionId=$('#iOrganisation_5').val();var iServiceId=$('#iOrganisation_6').val();var iDivisionId=$('#iOrganisation_7').val();if(iProvinceId==''||iProvinceId==0){var iTestValid=0;zMessage+="- Veuillez sélectionner une province\n"}
...
if(zPhone==''||zPhone==0){var iTestValid=0;zMessage+="- Veuillez remplir le téléphone\n"}
if(iTestValid==1){$.ajax({url:"{/literal}{$zBasePath}{literal}accueil/saveLocaliteServiceEvaluateur/",method:"POST",data:{iUserId:iUserId,iProvinceId:iProvinceId,iRegionId:iRegionId,iDistrictId:iDistrictId,iDepartementId:iDepartementId,iDirectionId:iDirectionId,iServiceId:iServiceId,iDivisionId:iDivisionId,iModeForChange:iModeForChange,zPhone:zPhone,zEmail:zEmail},success:function(data,textStatus,jqXHR){$("#dialog3").dialog("close");window.location.reload()},async:!1})}else{alert(zMessage)}}}]});$(".dialog-link-manuel-localite").click(function(event){$("#dialog3").html();var zTitle=$(this).attr("title");$('#dialog3').dialog('option','title','Confirmation localité de service');$('#buttonId').button('option','label','Valider');var iUserTarget=$("#iUserId").val();var iUserId=$("#iUserId").val();$.ajax({url:"{/literal}{$zBasePath}{literal}accueil/getInfoChangeManuel/"+iUserId,type:'post',data:{iUserTarget:iUserTarget},success:function(data,textStatus,jqXHR){$("#dialog3").html(data);$("#dialog3").dialog("open");event.preventDefault()},async:!1})});$(".dialog-link-manuel-localite").click();{/literal}{/if}{literal}
{/literal}{if $oData.bPhoto==0}{literal}
$("#dialog8").dialog({autoOpen:!1,width:'40%',closeOnEscape:!1,dialogClass:"noclose",modal:!0,open:function(){$('.ui-widget-overlay').addClass('custom-overlay');$.ui.dialog.prototype._allowInteraction=function(e){return!!$(e.target).closest('.ui-dialog, .ui-datepicker, .select2-drop').length}},show:{effect:"slide",duration:1000},hide:{effect:"drop",duration:1000},buttons:[{text:'Valider',"class":'saveButtonClass',click:function(){var iMiseADispo=0;var iTestValid=1;var zMessage="";var zPhoto=$('#photo').val();var zPhone=$('#zPhone').val();if(zPhoto==''){var iTestValid=0;zMessage+="- Veuillez sélectionner votre photo\n"}else{var ext=getExtension(zPhoto).toLowerCase();if(ext=="png"||ext=="gif"||ext=="jpg"||ext=="jpeg"){}else{var iTestValid=0;zMessage+="Veuillez entrer le fichier ayant des extensions .jpeg/.jpg/.png/.gif.";$('#photo').val("")}}
if(zPhone==''){var iTestValid=0;zMessage+="- Veuillez remplir le telephone\n"}
if(iTestValid==1){$("#photoMAJ").submit()}else{alert(zMessage)}}}]});$(".dialog-link-manuel-localite11").click(function(event){$("#dialog8").html();$('#dialog8').dialog('option','title','Mise à jour photo et Telephone dans ROHI');$('#buttonId').button('option','label','Valider');var iUserTarget=$("#iUserId").val();var iUserId=$("#iUserId").val();$.ajax({url:"{/literal}{$zBasePath}{literal}avis/getTemplatePhoto/"+iUserId,type:'post',data:{iUserTarget:iUserTarget,iRedirect:1},success:function(data,textStatus,jqXHR){$("#dialog8").html(data);$("#dialog8").dialog("open");event.preventDefault()},async:!1})});$(".dialog-link-manuel-localite11").click();{/literal}{/if}{literal}
})
</script>
{/literal}
</body>
