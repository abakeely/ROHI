<nav>
	...
	<ul>
		...
		{if $oModules.module_id < 4 || $oModules.module_id >= 6}
			<li {if $iModuleActif == $oModules.module_id} class="active" {/if} ><a href="{ if $oModules.module_id==1}{$zBasePath}accueil/index {else}{$zBasePath}gcap/module/{$oModules.module_zHashUrl}{/if}">{$oModules.module_libelle}<span><i class="fa {$oModules.module_icone}"></i></span></a></li>
			...
			{/if}
		{else}
			{if $iSessionCompte == COMPTE_ADMIN && $oModules.module_id ==4}
