				...
				$.ajax({
					...
					success: function(data, textStatus, jqXHR) {
							...
							if (oReturn.user_id == 0) {
								...
								$("#spanDialog").html('<br/><br/><br/>l\'agent ' + oReturn.zName + ' est déjà disponible dans votre liste<br/><br/><br/>');
								$("#dialog4").dialog( "open" );
								$("#iUserAddNew").val(0);
								$("#iModeAddUser").val(0);
							}
...
function getTable(_iMode) {
	...
	$(document).ready (function ()
	{
		...
		$.ajax({
			...
			method: "POST",
			data: { iModeId:_iMode,iPeriode:iPeriode,iAnnee:iAnnee},
			success: function(data, textStatus, jqXHR) {
				var oReturn = jQuery.parseJSON(data);
				...
				click: function() {
					...
					if (iTestValid == 1) {
							...
							$.ajax({
								...
								method: "POST",
								data: { iUserId:iUserId,iProvinceId:iProvinceId,iRegionId:iRegionId,iDistrictId:iDistrictId,iDepartementId:iDepartementId, iDirectionId:iDirectionId, iServiceId:iServiceId, iDivisionId:iDivisionId,iModeForChange:iModeForChange},
								success: function(data, textStatus, jqXHR) {
