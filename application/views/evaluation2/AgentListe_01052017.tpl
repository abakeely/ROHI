...
<script>
function getTable(_iMode){$(document).ready(function()
{$.ajax({url:"{/literal}{$zBasePath}{literal}critere/getListeANoterOuDejaNoter",method:"POST",data:{iModeId:_iMode},success:function(data,textStatus,jqXHR){var oReturn=jQuery.parseJSON(data);$("#tab-"+_iMode).html(oReturn.zHtmlReturn);if(oReturn.iSpan1>0){$("#span_1").html(oReturn.iSpan1);$("#span_1").show()}else{$("#span_1").hide()}
if(oReturn.iSpan2>0){$("#span_2").html(oReturn.iSpan2);$("#span_2").show()}else{$("#span_2").hide()}},async:!1})})}
$(document).ready(function()
{$('ul.tabs li').click(function(){var tab_id=$(this).attr('data-tab');var iModeId=$(this).attr('iModeId');$('ul.tabs li').removeClass('current');$('.tab-content').removeClass('current');$(this).addClass('current');$("#"+tab_id).addClass('current');getTable(iModeId)})
$("#dialog").dialog({autoOpen:!1,width:'90%',title:'Fiche note évaluation',close:'X',modal:!0,buttons:[{text:"Valider",click:function(){iValueEvaluable=$("#iEvaluableValue").val();if(iValueEvaluable==1){iClassificationValue=$("#iClassificationValue").val();if(iClassificationValue==0){alert("Veuillez choisir une catégorie")}else{iTestNote=1;var zAllNote="";var zMessageError="";iValue=$('#iManuel').is(':checked');iMoyenneUserInfoPointage=$("#iMoyenneUserInfoPointage").val();$('.lesNotes').each(function()
{if(zAllNote==''){zAllNote+=$(this).attr("zCritereId")+"-"+$(this).val()}else{zAllNote+=';'+$(this).attr("zCritereId")+"-"+$(this).val()}
if($(this).val()==''&&$(this).attr("zCritereId")!=7)
{iTestNote=0;zMessageError+="- Veuillez sélectionner la note : "+$(this).attr("zLibelle")+"\n"}
if(($(this).attr("zCritereId")==7&&iValue==!0&&$(this).val()=='')||($(this).attr("zCritereId")==7&&iValue==!1&&iMoyenneUserInfoPointage==''&&$(this).val()==''))
{iTestNote=0;zMessageError+="- Veuillez sélectionner la note : "+$(this).attr("zLibelle")+"\n"}})
...
getTable(1);$("#dialog").dialog("close")},async:!1})}}else{alert("Veuillez sélectionner l'Evaluateur!")}}}},{text:"Annuler",click:function(){$(this).dialog("close")}}]});$("#dialog2").dialog({autoOpen:!1,width:'50%',title:'Validation changement localité de service',close:'X',modal:!0,buttons:[{text:"Valider",click:function(){var iUserId=$("#iUserId").val();$.ajax({url:"{/literal}{$zBasePath}{literal}critere/saveLocaliteService/",type:'Post',data:{iUserId:iUserId},success:function(data,textStatus,jqXHR){$("#iLocalChangeAgentId_"+iUserId).hide();$("#dialog2").html();$("#dialog2").dialog("close");event.preventDefault()},async:!1})}},{text:"Annuler",click:function(){$(this).dialog("close")}}]});$("#dialog4").dialog({autoOpen:!1,width:'60%',title:'Notification',close:'X',modal:!0,buttons:[{text:"Valider",click:function(){var iTest=0;var zListe="";$('.approuver').each(function()
{var iValue=$(this).is(':checked');if(iValue==!0){iUserId=$(this).attr("getUserId");if(zListe==''){zListe+=iUserId;iTest=1}else{zListe+="-"+iUserId;iTest=1}}})
if(iTest==0){alert("Veuillez cocher au moins un agent si vous voulez valider")}else{$.ajax({url:"{/literal}{$zBasePath}{literal}critere/saveUpdateUser/",method:"POST",data:{zListe:zListe},success:function(data,textStatus,jqXHR){$("#dialog4").dialog("close");document.location.href='{/literal}{$zBasePath}{literal}critere/liste/agent-evaluation/a-evaluer/2'},async:!1})}}},{text:"Annuler",click:function(){$(this).dialog("close")}}]});$("#dialog3").dialog({autoOpen:!1,width:'75%',title:'Changement manuel localité de service d\'un agent',close:'X',modal:!0,buttons:[{text:"Valider",click:function(){var iTestValid=1;var zMessage="";var iUserId=$("#iUserId").val();var iDepartementId=$('#iDepartementId').val();var iDirection=$('#iOrganisation_1').val();var iService=$('#iOrganisation_2').val();var iDivision=$('#iOrganisation_3').val();var iDesevaluer=$("#iDesevaluer").val();var zMotifSuppression=$("#zMotifSuppression").val();if(iDepartementId==''||iDepartementId==0){var iTestValid=0;zMessage+="- Veuillez sélectionner un département\n"}
if(iDirection==''||iDirection==0){var iTestValid=0;zMessage+="- Veuillez sélectionner une direction\n"}
if(iService==''||iService==0){var iTestValid=0;zMessage+="- Veuillez sélectionner un service\n"}
if(iDesevaluer==1){if(zMotifSuppression==''){var iTestValid=0;zMessage+="- Veuillez remplir le motifs de la suppresion\n"}}
if(iTestValid==1){$.ajax({url:"{/literal}{$zBasePath}{literal}critere/saveLocaliteServiceEvaluateur/",method:"POST",data:{iUserId:iUserId,iDepartementId:iDepartementId,iDirection:iDirection,iService:iService,iDivision:iDivision,iDesevaluer:iDesevaluer,zMotifSuppression:zMotifSuppression},success:function(data,textStatus,jqXHR){$("#dialog3").dialog("close");window.location.reload()},async:!1})}else{alert(zMessage)}}},{text:"Annuler",click:function(){$(this).dialog("close")}}]});$(".dialog-link").click(function(event){$("#userANoteId").val("");$("#noteOfUserId").val("");$("#dialog").html();var iUserId=$(this).attr("iAgentId");$.ajax({url:"{/literal}{$zBasePath}{literal}critere/getInfoUser/"+iUserId,type:'get',data:{iEvaluateur:1},success:function(data,textStatus,jqXHR){$("#dialog").html(data);$("#dialog").dialog("open");event.preventDefault()},async:!1})});$(".dialog-link-localite").click(function(event){$("#dialog2").html();var iUserId=$(this).attr("iAgentId");$.ajax({url:"{/literal}{$zBasePath}{literal}critere/getInfoUserChangeLocalite/"+iUserId,type:'get',data:{iEvaluateur:1},success:function(data,textStatus,jqXHR){$("#dialog2").html(data);$("#dialog2").dialog("open");event.preventDefault()},async:!1})});$(".dialog-link-manuel-localite").click(function(event){$("#dialog3").html();var iUserId=$(this).attr("iAgentId");$.ajax({url:"{/literal}{$zBasePath}{literal}critere/getInfoChangeManuel/"+iUserId,type:'get',data:{iEvaluateur:1},success:function(data,textStatus,jqXHR){$("#dialog3").html(data);$("#dialog3").dialog("open");event.preventDefault()},async:!1})});$(".dialog-link, #icons li").hover(function(){$(this).addClass("ui-state-hover")},function(){$(this).removeClass("ui-state-hover")});{/literal}
{if sizeof($oData.toListeApprouver)>0&&$oData.iCurrPage==1}
{literal}
$("#dialog4").dialog("open");$("#dataTables-dialog4").dataTable();{/literal}
{/if}
{literal}
{/literal}
{if sizeof($oData.oListe)>0}
{literal}
$("#table-liste-evaluer-1").dataTable();{/literal}
{/if}
{literal}})
</script>
{/literal}
