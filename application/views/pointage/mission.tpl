			...
			{include_php file=$zHeader}
					...
					<div class="page-header">
						<div class="row align-items-center">
							<div class="col-12">
								<h3 class="page-title">Mission Ext&eacute;rieur : {$oData.oCandidat.0->nom}&nbsp;{$oData.oCandidat.0->prenom}</h3>
								<ul class="breadcrumb">
										...
										<fieldset>
											...
											<div class="row clearfix">
												<div class="cell small">
													<div class="field">
														<label>Date</label>
														<input type="text" data-dd-opt-format="dd/mm/y" data-dd-opt-default-date="{$oData.date|date_format2}" name="zDate" id="zDate" value="{$oData.date}" placeholder="s&eacute;l&eacute;ctionner la date" class="withDatePicker obligatoire">
								...
								<div class="col-xs-12">
									<table id="dataTableHistoriqueEntree">
										...
										<tbody>
											...
											{if sizeof($oData.toMission)>0}
											{foreach from=$oData.toMission item=oListe }
											<tr {if $iIncrement%2 == 0} class="even" {/if}>
												<td>{$oListe.nom}</td>
												<td>{$oListe.mission_date|date_format:"%d/%m/%Y"}</td>
												<td>{$oListe.mission_heureSortie}</td>
												<td id="iDateChange_{$oListe.mission_id}">{$oListe.mission_dateEntree|date_format:"%d/%m/%Y"}</td>
												<td id="iLocalChange_{$oListe.mission_id}">{$oListe.mission_heureEntree}</td>
												<td>{$oListe.mission_motif}</td>
												<td class="center">
													{if $oListe.mission_heureEntree == ''}
													<a title="Modification Heure de entr&eacute;e" alt="Modification Heure d'entr&eacute;e" style="cursor:pointer;" title="" id="hour_{$oListe.mission_id}" iMissionId="{$oListe.mission_id}" class="action dialog-link-heure"><i style="color:#12105A" class="la la-clock-o"></i></a>
													{/if}
												</td>
											</tr>
...
$(document).ready (function ()
{
		...
		buttons: [
			{
				...
				click: function() {
					...
					if (zMessage != ''){
						...
						$.ajax({
							url: "{/literal}{$zBasePath}{literal}pointage/saveHourUserMission/" ,
							type: 'Post',
							data: { iUserId:iUserId, iMissionId:iMissionId,zHeureEntree:zHeureEntree, zMinuteEntree:zMinuteEntree, zSecondeEntree:zSecondeEntree, zDateEntrer:zDateEntrer},
	...
	$( ".dialog-link-heure" ).click(function( event ) {
		...
		var iMissionId = $(this).attr("iMissionId");
		$.ajax({
