...
<div id="container">
	<div id="breadcrumb"><a href="{$zBasePath}">Accueil</a> <span>&gt;</span> <a href="#">Formation</a> <span>&gt;</span> <a href="{$zBasePath}formation/texte/sfao/texte-reference">Liens utiles</a><span>&gt;</span>Documents points focaux</div>
    <section id="content">
        {include_php file=$zLeft}	
		<div id="innerContent">
		<div id="ContentBloc">
		<h2>Documents points focaux{if $oData.iSessionCompte == COMPTE_AUTORITE || $oData.iSessionCompte == COMPTE_ADMIN}
		&agrave; finaliser
...
{assign var=i value=$i+1}
	<td class="zoom">
		<div class = "classClick">
		 <br>
		  <a href="#liste{$i}">
			<img src="{$zBasePathRacine}/backoffice/assets/img/img_sfao/partenaire/doc_point/{$oPartenaire.partenaire_photo}" >
			<br><br>
		  </a>
			<div class="field">				
				<a href="#liste{$i}"><button>{$oPartenaire.partenaire_lib}</button></a>				
			</div>
	    </div>
            ...
            <table align="center" class="table table-bordered table-hover dataTable no-footer">
              ...
              <tbody>
				{foreach from=$oData.contenuformation  item=oContenu}
    				...
    				<tr>
                      <td style="text-align:left">{$oContenu.contenuformation_titre}</td>
						<td>
						<a target="blanck" class="la la-download" style="font-size: 2.5em;" style="font-size: 2.5em;" title="Télécharger" href="{$zBasePath}assets/pdf_sfao/bourse/{$oContenu.contenuformation_fichier_pdf}"></a>	
						</td>
					</tr>
					{/if}
				{/foreach}
