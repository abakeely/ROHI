...
<div id="container">
	<div id="breadcrumb"><a href="{$zBasePath}">Accueil</a> <span>&gt;</span> <a href="#">Formation</a> <span>&gt;</span> <a href="{$zBasePath}">Offres</a><span>&gt;</span> Inscription en ligne</div>
	{include_php file=$zHeader}
    <section id="content">
        {include_php file=$zLeft}	
		<div id="innerContent">
		<div id="ContentBloc">
		<h2>Inscription en ligne{if $oData.iSessionCompte == COMPTE_AUTORITE || $oData.iSessionCompte == COMPTE_ADMIN}
		&agrave; finaliser
...
<script>
    $(document).ready(function() {	 
	  ...
	  replace = function()
		...
		$('.classClick2').addClass('hidden');
		$('.classClick').removeClass('hidden');
	...
	$(document).on('click', '.zoom',function(e){
		...
		$('.classClick').removeClass('hidden');
         ...
         child = $(this).find('.classClick2');
			if( child.hasClass('hidden'))
		   {

		      $(this).find('.classClick').addClass('hidden');
		      child.removeClass('hidden');
		   }else{
		   		$(this).find('.classClick').removeClass('hidden');
