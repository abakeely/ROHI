 ...
 <div id="content-wrap" class="row"> 
	...
	<table class="table table-striped table-bordered table-hover" id="dataTables-example">
		...
		?>  
		 <tr>
			...
			<td style="cursor:pointer" ><?php echo $ordre; ?></td>
			<td style="cursor:pointer" ><?php echo ($candidat->statut)?$candidat->statut['libele']:''; ?></td>
			<td style="cursor:pointer" ><?php echo $candidat->matricule; ?></td>
			<td style="cursor:pointer" ><?php echo $candidat->nom.' '.$candidat->prenom;  ?></td>
			
						
			<td style="cursor:pointer" ><?php echo rohi_is_empty($candidat->departement)?'':$candidat->departement; ?></td>
			<td style="cursor:pointer" ><?php echo rohi_is_empty($candidat->direction)?'':$candidat->direction; ?></td>
			<td style="cursor:pointer" ><?php echo rohi_is_empty($candidat->service)?'':$candidat->service;?></td>
			<td style="cursor:pointer" ><?php echo rohi_is_empty($candidat->division)?'':$candidat->division;?></td>
			
			<td style="cursor:pointer"  ="openCV(<?php echo $candidat->id; ?>)"><?php echo isset($candidat->region['libele'])?$candidat->region['libele']:'';?></td>
			...
			<td><a onclick="openPopup('<?php echo $id;?>')" href="#"><i class="la la-reply-all" aria-hidden="true" tada-toggle="tooltip" data-original-title = "Changement des Informations Admin"></i></i></a></td>
			<!--Modif by lucia du 01_07_2016-->
			<td><a  href="fpdf_fe/<?php echo $id;?>" target="_blank"><i class="la la-file-pdf-o" aria-hidden="true" tada-toggle="tooltip" data-original-title = "Fiche Evaluation" ></i></i></a></td>
			<td><a  href="mon_cv?id=<?php echo $candidat->id;;?>" target="_blank"><i class="la la-file-text-o" aria-hidden="true" tada-toggle="tooltip" data-original-title = "CV" ></i></i></a></td>
  ...
  <script>
    $('i').tooltip();
    function openCV(id){
    	window.location = '<?php echo base_url();?>cv/mon_cv?id='+id; 
    }
