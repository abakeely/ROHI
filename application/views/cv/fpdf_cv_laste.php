...
$oImage = imagecreatefromjpeg($zDestination);
$iSize = min(imagesx($oImage), imagesy($oImage));

//echo "dsdsds " . imagesx($oImage) . " > " . imagesy($oImage) ;
...
$zImage = base_url().'assets/cv_ico/destination_'.$id.'.png';
...
$zImage = base_url().'assets/cv_ico/tirer.png';
...
$zImage = base_url().'assets/cv_ico/adresse.png';
...
$zImage = base_url().'assets/cv_ico/tel.png';
...
$zImage = base_url().'assets/cv_ico/mails.png';
...
$zImage = base_url().'assets/cv_ico/informations.png';
...
$link = mysqli_connect('p:'.$db['default']['hostname'], $db['default']['username'], $db['default']['password'], $db['default']['database']);
...
for ($iBoucle=1;$iBoucle<=8;$iBoucle++){
	...
	switch ($iBoucle){
		...
		case 1:
			$zImage = base_url().'assets/cv_ico/diplome_obtenue.png';
			$oPdf->Cell(130,15,$oPdf->Image($zImage, $oPdf->getX()+1, $oPdf->getY()+1, 127),$zBorder2,0,'C',0);
			$oPdf->Ln();
			break;

		case 2:
			$zSql = 'SELECT * FROM candidat_diplome where candidat_id = '.$id.' AND diplome_name !="" '; 
		...
		case 3:
			...
			if(sizeof($toStage)>0){
				$zImage = base_url().'assets/cv_ico/stage.png';
		...
		case 5:
			$zImage = base_url().'assets/cv_ico/experience.png';
			$oPdf->Cell(130,15,$oPdf->Image($zImage, $oPdf->getX()+1, $oPdf->getY()+1, 127),$zBorder2,0,'C',0);
			$oPdf->Ln();
			break;
		...
		case 6:
			$zSql = 'SELECT * FROM candidat_parcours where candidat_id = '.$id.' AND date_debut !="" '; 
			$zQuery = mysqli_query($link,$zSql);
			while ($oData = mysqli_fetch_array($zQuery)) {
				$oPdf->SetX(85);
		...
		case 7:
			$zImage = base_url().'assets/cv_ico/competences.png';
			$oPdf->Cell(130,15,$oPdf->Image($zImage, $oPdf->getX()+1, $oPdf->getY()+1, 127),$zBorder2,0,'C',0);
			$oPdf->Ln();
			break;
...
$oPdf->SetTextColor(0,0,0);
$oPdf->SetXY(130,287);
$oPdf->multicell(75,6,"© 2018 : http://rohi.mef.gov.mg:8088/ROHI",$zBorderLR,'C',1);
